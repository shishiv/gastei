  ...
  return (
    <div
      className={`relative rounded-2xl p-6 md:p-8 transition-all duration-300 hover:shadow-gastei-lg ${
        plan.highlighted
          ? 'bg-gradient-to-br from-gastei-green to-gastei-teal text-white shadow-gastei scale-105'
          : 'bg-white border border-border hover:border-primary/30'
      }`}
    >
      {plan.highlighted && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <span className="bg-gastei-orange text-white text-xs font-semibold px-3 py-1 rounded-full">
            Mais Popular
          </span>
        </div>
      )}
          ...
          <li key={index} className="flex items-start gap-3">
            <Check
              className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                plan.highlighted ? 'text-white' : 'text-gastei-green'
              }`}
            />
            <span
              className={`text-sm ${
      ...
      <Button
        className={`w-full rounded-full font-medium transition-all ${
          plan.highlighted
            ? 'bg-white text-gastei-green hover:bg-white/90'
            : 'bg-gastei-green text-white hover:bg-gastei-green-dark'
        }`}
      >
